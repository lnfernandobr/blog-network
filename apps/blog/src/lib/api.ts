...
/**
 * Canal placeholder retornado quando a API não está configurada.
 * Permite o build da Vercel passar mesmo antes da API estar no ar — o site
 * renderiza uma "casca" minimalista. Quando você plugar `NEXT_PUBLIC_API_URL`
 * + `NEXT_PUBLIC_CHANNEL_SLUG`, redeploya e tudo vira dado real.
 */
const PLACEHOLDER_CHANNEL: ChannelDto = {
  id: 'placeholder',
  slug: 'placeholder',
  name: 'Blog em construção',
  niche: 'em breve',
  siteUrl: SITE_URL,
  language: 'pt-BR',
  ...
  postsPerSlot: 1,
  publishWeekdays: [],
  defaultAuthorName: 'Equipe Editorial',
...
/**
 * Wrapper que torna toda chamada da API "tolerante":
 *   - se a env não estiver configurada, retorna o fallback sem fazer fetch
 *   - se o fetch falhar (API down), retorna o fallback e loga warning
 *
 * Pages do blog continuam renderizando, com dados vazios/placeholder.
 */
async function safeGet<T>(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.warn(`[blog] API call failed: ${path}`, err);
    }
    return fallback;
  }
}
